...
def _get_config(cluster_name):
    ...
    config_response = _admin_socket(mon_socket, ['config', 'show'], 'json')
...
def _cluster_status(cluster_handle, cluster_name):
    ...
    assert ret == 0
    pg_summary_digest = _md5(msgpack.packb(_pg_summary(json.loads(outbuf))))

    # Get digest of configuration
    config_digest = _md5(_get_config(cluster_name))
...
def get_heartbeats():
    ...
    for filename in glob("/var/run/ceph/*.asok"):
        ...
        config_response = _admin_socket(filename, ['config', 'get', 'fsid'], 'json')
        ...
        if service_type == 'mon':
            ...
            status = json.loads(_admin_socket(filename, ['mon_status'], 'json'))
        ...
        if ceph_version is None:
            version_response = _admin_socket(filename, ['version'], 'json')
    ...
    for fsid, socket_path in mon_sockets.items():
        ...
        admin_response = _admin_socket(socket_path, ['mon_status'], 'json')
        ...
        cluster_heartbeat[fsid] = _cluster_status(cluster_handle, fsid_names[fsid])
...
def get_cluster_object(cluster_name, sync_type, since):
    ...
    if sync_type == 'config':
        ...
        command, kwargs, version_fn = {
            ...
            'pg_summary': ('pg dump', {'dumpcontents': ['pgs_brief']}, lambda d, r: _md5(msgpack.packb(d))),
            'health': ('health', {'detail': ''}, lambda d, r: _md5(r))
        }[sync_type]
        kwargs['format'] = 'json'
        ...
        if sync_type == 'pg_summary':
            data = _pg_summary(json.loads(raw))
    ...
    for i, (prefix, argdict) in enumerate(commands):
        ...
        if ret != 0:
            return {
                ...
                'err_outbuf': outbuf,
                'err_outs': outs,
                'versions': _cluster_status(cluster_handle, cluster_name)['versions'],
                'fsid': fsid
            }
        if outbuf:
...
def heartbeat():
    ...
    _fire_event(service_heartbeat, 'ceph/server')
    for fsid, cluster_data in cluster_heartbeat.items():
        _fire_event(cluster_data, 'ceph/cluster/{0}'.format(fsid))
